...
namespace aq
{
    ...
    template<typename T> bool qualifier(mo::IParam* param, int width = -1, int height = -1, int channels = -1, int type = -1)
    {
        //auto typedParam = getParameterPtr<T>(param);
        auto typedParam_ = dynamic_cast<mo::ITParam<T>*>(param);
        if (typedParam_)
        {
    ...
    {
    public:
        static boost::function<bool(mo::IParam*)> get(int width = -1, int height = -1, int channels = -1, int type = -1)
        {
