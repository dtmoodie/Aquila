...
aq::CpuPinnedAllocator* aq::CpuPinnedAllocator::instance()
{
    static aq::CpuPinnedAllocator inst;
    return &inst;
}
cv::UMatData* aq::CpuPinnedAllocator::allocate(int dims, const int* sizes, int type,
...
bool aq::CpuPinnedAllocator::allocate(cv::UMatData* u, int /*accessFlags*/, cv::UMatUsageFlags /*usageFlags*/) const
{
    return (u != NULL);
}

void aq::CpuPinnedAllocator::deallocate(cv::UMatData* u) const
{
    if (!u)
        return;
