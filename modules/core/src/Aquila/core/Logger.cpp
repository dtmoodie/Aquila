...
std::map<std::string, std::shared_ptr<mo::TSignal<void(boost::log::trivial::severity_level, std::string)>>> object_specific_signals;
mo::TSignal<void(boost::log::trivial::severity_level, std::string)>                                         generic_signal;
mo::TSignal<void(boost::log::trivial::severity_level, std::string, std::string)>                            object_signal;
...
mo::TSignal<void(boost::log::trivial::severity_level, std::string)>& ui_collector::get_object_log_handler(std::string name)
{
    auto& sig = object_specific_signals[name];
    if(!sig)
        sig.reset(new mo::TSignal<void(boost::log::trivial::severity_level, std::string)>());
    return (*sig);
}
mo::TSignal<void(boost::log::trivial::severity_level, std::string, std::string)>& ui_collector::get_object_log_handler()
{
    return object_signal;
}
mo::TSignal<void(boost::log::trivial::severity_level, std::string)>& ui_collector::get_log_handler()
{
