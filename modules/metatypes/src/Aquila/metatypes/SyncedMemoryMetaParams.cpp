...
bool TInputParamPtr<aq::SyncedMemory>::getInput(OptionalTime_t ts, size_t* fn_) {
    ...
    if (_user_var) {
        ...
        if (ITInputParam<aq::SyncedMemory>::_shared_input) {
            ...
            cs = ITInputParam<aq::SyncedMemory>::_shared_input->getCoordinateSystem();
        }
        if (ITInputParam<aq::SyncedMemory>::_input) {
            if (!ITInputParam<aq::SyncedMemory>::_input->getData(data, ts, this->_ctx, &fn)) {
                return false;
            }
            cs = ITInputParam<aq::SyncedMemory>::_input->getCoordinateSystem();
        }
        _current_data = data;
        *_user_var    = ParamTraits<aq::SyncedMemory>::ptr(_current_data);
        if (fn_)
        ...
        if (ITInputParam<aq::SyncedMemory>::_shared_input) {
            ...
            if (ITInputParam<aq::SyncedMemory>::_shared_input->getData(data, fn, this->_ctx, &ts)) {
                ...
                *_user_var = ParamTraits<aq::SyncedMemory>::ptr(_current_data);
                ...
                this->_ts = ts;
                this->_fn = fn;
                this->setCoordinateSystem(ITInputParam<aq::SyncedMemory>::_shared_input->getCoordinateSystem());
                return true;
            }
        }
        if (ITInputParam<aq::SyncedMemory>::_input) {
            ...
            if (this->_input->getData(data, fn, this->_ctx, &ts)) {
                ...
                *_user_var    = ParamTraits<aq::SyncedMemory>::ptr(_current_data);
                ...
                this->_ts = ts;
                this->_fn = fn;
                this->setCoordinateSystem(ITInputParam<aq::SyncedMemory>::_input->getCoordinateSystem());
                return true;
            }
        }
    }
